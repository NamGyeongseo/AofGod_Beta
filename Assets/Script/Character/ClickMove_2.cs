...
{
    public Camera cam;
    public List<Character_2> selectCharactor = new List<Character_2>();
    ...
    private void OnMouseDown()
    {
        if (Input.GetMouseButtonDown(0)) // 마우스 좌클릭을 했을 때
        {
            ...
            {
                Character_2 character = hit_fir.collider.GetComponent<Character_2>();

                if (hit_fir.collider.gameObject.tag == "Character") // 태그가 캐릭터이면 실행
                {
                    Debug.Log("캐릭터 클릭!");
                    if (character != null && !selectCharactor.Contains(character))
                    {
                ...
                else if (selectCharactor.Count > 0 && hit_fir.collider.gameObject.tag == "Floor") // 바닥 클릭 시
                {
                    ...
                    foreach (Character_2 character1 in selectCharactor)
                    {
                        character1.MoveEvent(hit_fir.point); // 캐릭터들을 이동
                    }
                }
                else if (selectCharactor.Count > 0 && hit_fir.collider.gameObject.tag == "Desk_1"
                    || hit_fir.collider.gameObject.tag == "Flower_1") //오브젝트 클릭 시
                {
                    move = true;
                    work = true;
                    Debug.Log("오브젝트 클릭!");
    ...
    public void Remaining_()
    {
        ...
        {
            foreach (Character_2 character in selectCharactor)
            {
                // 이동이 완료되었는지 확인
